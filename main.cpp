...
void start() {
	...
	while (choose != 2) {
		...
		if (choose == 1) {
			...
			bool enter = 0;
			bool enterInCalc = 0;
			if (divideSymbols(stOfNumb, stOfChars, item )&& stOfChars.size + stOfNumb.size>=3)//if true
			{
				enter = 1;
				bool result(1);
				while (stOfChars.size != 0) {
					...
					if (calculating(stOfNumb, stOfChars, item) == false) {
						result = false;system("pause");
						break; 
					}
				}
				if (result && enter) { answer(stOfNumb); }// - )2  
			}
			if (enter && !enterInCalc) { std::cerr << "insufficient data" << std::endl; std::cin.ignore(); continue; }
			if (!enter) { std::cerr << "insufficient data" << std::endl; std::cin.ignore(); continue; }
		}
		else break;
	}
}
